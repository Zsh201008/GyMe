...
public class TAOperator implements ActionListener{
	...
	public TAOperator(String trainerID,String[] pair) {
	...
	public JPanel createPanel() {
		...
		try {
				...
				for(int i=0;i<this.pairList.length;i++) {
					if(this.pairList[i].equals(notiAttr[1])) {
						...
						if(notiAttr[2].equals("live")) {
							LiveSession ls=new LiveSession(readLive(notiAttr[3]),notiAttr[3]);
			...
			if(isCourseFound) {
				...
				myCoursePanel.setLayout(new GridLayout(5,0,5,5));
				JLabel noContentLabel=new JLabel("No recorded courses found",JLabel.CENTER);
				myCourseContentPanel.add(noContentLabel);
				myCoursePanel.add(myCourseTitlePanel);
				myCoursePanel.add(myCourseContentPanel);
			}
