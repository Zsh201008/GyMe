	...
	public int checkIdentity(String id, String pw, String identity) throws Exception{
		...
		while(newline!=null) {
			...
			if(items[0].equals(id)) {
				String pwMD5=MD5.getMD5(pw,32);
				if(identity.equals("User")) {
					if(items[1].equals(pwMD5))
						isValid=1;
				}
				else if(identity.equals("Trainer")) {
					if(items[5].equals(pwMD5))
						isValid=2;
				}	
				else {
					if(items[1].equals(pwMD5))
		...
		if(e.getSource()==this.loginIdentity) {
			...
			try {
				...
				if(isIdentity==1) {
					...
					TrainerPage tp=new TrainerPage(thisID);
					dispose();
				}
				else if(isIdentity==3) {
					AdminPage ap=new AdminPage(thisID);
					dispose();
				}
				else
					fail=new ErrorMessage("Wrong ID or password!");
			}
			catch(Exception ex) {
