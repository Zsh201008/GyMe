/**
 * Title: UTController.java
 * Controller class of user interacting with trainer.
 * @author Shuhan Zhang
 * @version 5.0
 */
import java.io.*;
...
public class UTController implements ActionListener {
	...
	public JPanel noTPanel() {
		...
		this.update=new JButton("Refresh Page");
		this.update.addActionListener(this);
		this.update.setBackground(titlec);
		buttonPane.add(this.update);
		...
		noTrainerPanel.setBackground(lightc);
	...
	public JPanel createMyTPane(String tID) {
		...
		try {
			...
			if(this.trainerNum==0)
				...
				while(bookLine!=null&&bookCount<4) {
					...
					if(bookAttr[1].equals(this.pairList[currentTrainer])) {
						if(bookAttr[5].equals("on")) {
							...
							LiveSession ls=new LiveSession(bookAttr[1],booking[bookCount],bookAttr[0],"cancel");
				...
				String[] recoTime=new String[2];
				String[] recoPeriod=new String[2];
				...
				while(recoLine!=null&&j1<2) {
					...
					if(recoAttr[0].equals(this.pairList[currentTrainer])) {
						...
						recoTime[j1]=recoAttr[2];
						recoPeriod[j1]=recoAttr[3];
						j1++;
					}
					...
					while(vLine!=null) {
						...
						for(int i2=0;i2<j1;i2++) {
							if(recoList[i2].equals(vAttr[0])) {
								Video v=new Video(this.userID,vAttr[0],vAttr[1],vAttr[2],vAttr[3],vAttr[4],180,100,recoTime[i2],recoPeriod[i2]);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==this.addTrainer) {
			...
			BookLive bl=new BookLive(this.userID,this.pairList[currentTrainer]);
		}
		...
		public void deleteTrainer() {
			try {
				...
				while(utLine!=null) {
					...
					if(!utAttr[2].equals(trainerIDs.get(currentTrainer))) {
				...
				String notiID="n"+nid;
				String wString=notiID+","+this.pairCID+",delete,null";
	...
	public class LiveSession implements ActionListener{
		...
		public LiveSession(String pairID, String booking, String id, String type) {
			this.pairID=pairID;
			this.opType=type;
			this.bookID=id;
				...
				try {
					...
					while(bookLine!=null&&bookCount<MAX_BOOKING) {
						...
						if(!this.bookID.equals(bookAttr[0])) {
							...
							currentBook=bookAttr[2]+" on "+bookAttr[3];
					...
					int nid=(int)(Math.random()*10000)+10000;
					String notiID="n"+nid;
					String wString=notiID+","+pairID+",cancel,"+currentBook;
					NotiToFile ntf=new NotiToFile("trainer",wString);
					boolean isOK=ntf.notiToFile();
					if(isOK) {
