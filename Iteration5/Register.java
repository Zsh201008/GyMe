/**
 * Title: Register.java
 * Class of performing registration.
 * @author Shuhan Zhang
 * @version 5.0
 */
import java.awt.*;
...
//		imgLabel.setBounds(0, 0, 500, 550);
	...
	public String writeToFile() throws Exception {
		...
		String wpwMD5=MD5.getMD5(wpassword,32);
		...
		if(wid.equals("User")) {
			...
			if(ValidityCheck(wname, wpassword, wdob, wtelephone, waddress) == 0) {
				String wString=wacc+","+wpwMD5+","+wname+","+wsex+","+wdob+","+wtelephone+","+rank+","+type+","+waddress+","+wclassnum+","+wtime+","+wAvatar+"\n";
				...
				ErrorMessage em=new ErrorMessage("Invalid Input Info!");
				return "";
			}
		}
		else {
			...
			if(ValidityCheck(wname,wpassword,wdob,wtelephone,waddress)==0) {
				String wString=wacc+","+wname+","+wsex+","+focus+","+wAvatar+","+wpwMD5+","+wdob+","+wtelephone+","+waddress+","+wclassnum+","+wtime+"\n";
				RandomAccessFile raf=new RandomAccessFile(file, "rw");
				raf.seek(raf.length());
				raf.writeBytes(wString);
				raf.close();
